export function verifyGraphicManifest(manifest) {
    ...
    if (typeof manifest.rendering.supportsRealTime !== 'boolean') errors.push( 'Manifest "rendering.supportsRealTime" property must be a boolean')
    if (typeof manifest.rendering.supportsNonRealTime !== 'boolean') errors.push( 'Manifest "rendering.supportsNonRealTime" property must be a boolean')

    if (manifest.description && typeof manifest.description !== 'string') errors.push( 'Manifest "description" property must be a string')
    if (manifest.author && typeof manifest.author !== 'object') errors.push( 'Manifest "author" property must be an object')

    if (manifest.author) {
        if (typeof manifest.author.name !== 'string') errors.push( 'Manifest "author.name" property must be a string')
        if (manifest.author.email && typeof manifest.author.email !== 'string') errors.push( 'Manifest "author.email" property must be a string')
        if (manifest.author.url && typeof manifest.author.url !== 'string') errors.push( 'Manifest "author.url" property must be a string')
    }
    ...
    for (const [key, action] of Object.entries(manifest.actions)) {
        ...
        if (action.description && typeof action.description !== 'string') errors.push( `Manifest "actions.${key}.description" property must be a string`)
    }
