...
async function main() {
  ...
  for (const folder of await fs.promises.readdir(
    path.join(basePath, graphicsPath)
  )) {
    ...
    if (process.platform === "win32") {
      await run({
        ...
        label: `${folderPath}`,
        cmd: `powershell.exe`,
        args: ["Compress-Archive", "-Path", "*", "-DestinationPath", zipName],
        cwd: folderPath,
      });
    } else {
      await run({
    ...
    if (await exists(path.join(basePath, cwd))) {
      ...
      return new Promise((resolve, reject) => {
        ...
        ls.on("close", (code) => {
          ...
          if (code !== 0)
            reject(
              new Error(`[${label}] "${cmd}" closed with exit code ${code}`)
            );
          else resolve();
        });
      });
      ...
      log(
        label,
        `${consoleColors.FgYellow}Skipping, folder not found: ${cwd}${consoleColors.Reset}`
      );
    }
  } finally {
    runningCount--;
